...
class AmazonS3ServiceTest {
    ...
    @Test
    void givenNonExistentBucketName_whenGettingBucketObjects_thenReturn404NotFoundAndCorrectMessage() {
        ...
        assertThat(response.getBody()).isEqualTo(String.format("Bucket '%s' does not exist", bucket));
        verify(amazonS3Client, times(1)).listObjects(bucket);
    }

    @Test
    void givenAmazonServiceExceptionWithOtherErrorCode_whenGettingBucketObjects_thenReturn500InternalServerError() {
    ...
    // getObject
    @Test
    void givenValidBucketAndKey_whenGettingObject_thenReturnStreamingResponseWithCorrectHeaders() throws Exception {
        String bucket = "valid-bucket";
        ...
        S3ObjectInputStream s3ObjectInputStream = new S3ObjectInputStream(
                new ByteArrayInputStream("dummy data".getBytes()), null);
        ...
        assertThat(response.getHeaders().getFirst("Content-Length")).isEqualTo(String.valueOf(fileLength));
        assertThat(response.getHeaders().getFirst("Content-Disposition")).isEqualTo("attachment; filename=\"video.mp4\"");
    ...
    @Test
    void givenAmazonServiceExceptionWithInternalError_whenGettingObject_thenReturn500InternalServerError() {
        String bucket = "any-bucket";
    ...
    @Test
    void givenS3Object_whenGetStreamingResponseBody_thenStreamsAllBytesAndClosesS3Object() throws Exception {
        // Given
        S3Object s3Object = mock(S3Object.class);
        ...
        var method = AmazonS3Service.class.getDeclaredMethod("getStreamingResponseBody", S3Object.class);
